...
export function LocationCard({ venue, address, pickupLocation, delay = 0.1 }: LocationCardProps) {
  const [isHovered, setIsHovered] = useState(false);

  // Generate Google Maps directions URL
  const getDirectionsUrl = () => {
    const destination = encodeURIComponent(`${venue}, ${address}`);
    const origin = pickupLocation ? encodeURIComponent(pickupLocation) : '';

    if (origin) {
      // With origin (pickup location) specified
      return `https://www.google.com/maps/dir/?api=1&origin=${origin}&destination=${destination}&travelmode=driving`;
    } else {
      // Without origin - opens location in Google Maps
    ...
    >
      <div className="flex items-start gap-3 sm:gap-4">
        ...
        <div className="flex-1">
          ...
          >
            {pickupLocation ? `Route: ${pickupLocation} → Destination` : 'Location'}
          </motion.p>
          ...
          >
            ...
            <span>{pickupLocation ? 'Get Directions from Pickup' : 'Get Directions'}</span>
            <motion.span
              animate={{
                x: isHovered ? 4 : 0,
              }}
